...
def __init__(self, readRef):
    ...
    def submit(expression):
        ...
        for i in range(len(selectedEntry)):
            ...
            erroriData = ax_1[1, 0].errorbar(dataToPlot[selectedEntry[i]][0], dataToPlot[selectedEntry[i]][1],
                                             label='entry' + str(selectedEntry[i]),
                                             yerr=dataToPlot[selectedEntry[i]][2], color=next(colors))
